...
const Stats = {
  ...
  update(rearrangements, t0, t1) {
    let timeSeached = parseFloat(t1 - t0)
      .toFixed(0)
      .toString();
...
function formatTime(time) {
  ...
  else if (time.length < 4) return '0.' + time.slice(1);

  let result = [];
  const arr = time.split('');
  for (let i = 0; i < arr.length - 1; i++) {
    if (i === arr.length - 3) result.push('.');
...
class Line {
  ...
  changeMoveColor() {
    ...
    this.graphic.drawRect(this.xPos, this.yPos, this.width, this.height);
...
function drawRightNav() {
  ...
  let baseYPos = 100;
  const bubbleSortBox = new CheckBox(
    'Bubble Sort',
    bubbleSort,
    BlackFont,
    rightNavXPos + 20,
  ...
  const insertionsSortBox = new CheckBox(
    'Insertion Sort',
    insertionSort,
    BlackFont,
    rightNavXPos + 20,
  ...
  const selectSortBox = new CheckBox(
    'Select Sort',
    selectSort,
    BlackFont,
    rightNavXPos + 20,
  ...
  const quickSortBox = new CheckBox(
    'Quick Sort',
    quickSort,
    BlackFont,
    rightNavXPos + 20,
  ...
  const countingSortBox = new CheckBox(
    'Counting Sort',
    countingSort,
    BlackFont,
    rightNavXPos + 20,
...
class CheckBox {
  ...
  selectCheckBox() {
    ...
    setupLineContain(this.lineWidth, this.numberOfLines, this.lineHeightDiff);
...
async function quickSort(array) {
  ...
  for (let i = 0; i < array.length; i++) {
    const leftLine = leftArray.filter((e) => {
      return e.height === array[i].height;
    });
    const rightLine = rightArray.filter((e) => {
      return e.height === array[i].height;
    });
    changeToDefaultColors();
  ...
  if (leftArray.length > 0 && rightArray.length > 0)
    result = [
      ...(await Promise.all([quickSort(leftArray)])),
      pivot,
      ...(await Promise.all([quickSort(rightArray)])),
    ];
  else if (leftArray.length > 0)
    result = [...(await Promise.all([quickSort(leftArray)])), pivot];
  else result = [pivot, ...(await Promise.all([quickSort(rightArray)]))];
